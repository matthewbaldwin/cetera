...
class SearchServiceSpecForUsersWithStoriesRights
  ...
  test("searching across all domains finds " +
    ...
    val dom = domains(0)
    val storiesWithinDomain = docs.filter(d => d.socrataId.domainId == dom.id && d.isStory).map(d => d.socrataId.datasetId)
    val internalNonStoriesWithinDomain = internalDocs.filter(d => d.socrataId.domainId == dom.id && !d.isStory).map(d => d.socrataId.datasetId)
